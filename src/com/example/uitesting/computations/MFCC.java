...
public class MFCC {   
    ...
    /**Sampling frequency.  
     */   
    private final float m_dsamplingFrequency;   
     ...
     * here.  
     */   
    private final float m_dminimumFilterOutput = 1.0f;   
     ...
     * here.  
     */   
    private final float m_dlogFilterOutputFloor = 0.0f;   
    private int[][] m_nboundariesDFTBins;   
    private float[][] m_dweights;   
    private FFT m_fft;   
    private float[][] m_ddCTMatrix;   
   
    private float[] m_dfilterOutput;   
    private final float[] m_nlifteringMultiplicationFactor;   
   
    //things to be calculated just once:   
    private final float m_dscalingFactor;   
    ...
    public MFCC(int nnumberOfParameters,   
        ...
        m_dsamplingFrequency = (float) dsamplingFrequency;   
        ...
        m_dfilterOutput = new float[m_nnumberOfFilters];   
        ...
        m_dscalingFactor = (float) Math.sqrt(2.0 / m_nnumberOfFilters);   
        ...
        if (m_oisLifteringEnabled) {   
            ...
            //it is not liftered   
            m_nlifteringMultiplicationFactor = new float[m_nlifteringCoefficient];   
            float dfactor = (float) (m_nlifteringCoefficient / 2.0);   
            float dfactor2 = (float) (Math.PI / m_nlifteringCoefficient);   
            for (int i=0; i<m_nlifteringCoefficient; i++) {   
                m_nlifteringMultiplicationFactor[i] = (float) (1.0 + dfactor * Math.sin(dfactor2*(i+1)));   
            }   
    ...
    private void initializeDCTMatrix() {   
        m_ddCTMatrix = new float[m_nnumberOfParameters][m_nnumberOfFilters];   
        for(int i=0;i<m_nnumberOfParameters;i++) {   
            for(int j=0;j<m_nnumberOfFilters;j++) {   
                m_ddCTMatrix[i][j] = (float) Math.cos((i+1.0)*(j+1.0-0.5)*(Math.PI/m_nnumberOfFilters));   
            }   
        }   
    }   
     ...
     */   
    public static float[] convertHzToMel(float[] dhzFrequencies, double dsamplingFrequency) {   
        float[] dmelFrequencies = new float[dhzFrequencies.length];   
        for (int k=0; k<dhzFrequencies.length; k++) {   
            dmelFrequencies[k] = (float) (2595.0*( Math.log(1.0 + (dhzFrequencies[k] / 700.0) ) / Math.log(10) ));   
        }   
    ...
    private void calculateMelBasedFilterBank(double dsamplingFrequency,   
        ...
        float[][] dfrequenciesInMelScale = new float[nnumberofFilters][3];   
        //the +1 below is due to the sample of frequency pi (or fs/2)   
        float[] dfftFrequenciesInHz = new float[nfftLength/2 + 1];   
        //compute the frequency of each FFT sample (in Hz):   
        float ddeltaFrequency = (float) (dsamplingFrequency / nfftLength);   
        for (int i=0; i<dfftFrequenciesInHz.length; i++) {   
            dfftFrequenciesInHz[i] = i * ddeltaFrequency;   
        }   
        //convert Hz to Mel   
        float[] dfftFrequenciesInMel = this.convertHzToMel(dfftFrequenciesInHz,dsamplingFrequency);   
        ...
        float[] dfilterCenterFrequencies = new float[nnumberofFilters + 2];   
        ...
        m_dweights = new float[m_nnumberOfFilters][];   
        ...
        //allocate space   
        for(int i=0;i<nnumberofFilters;i++) {   
            m_dweights[i] = new float[m_nboundariesDFTBins[i][1]-m_nboundariesDFTBins[i][0]+1];   
        }   
        ...
        for(int i=1;i<=nnumberofFilters;i++) {   
            for(int j=m_nboundariesDFTBins[i-1][0],k=0;j<=m_nboundariesDFTBins[i-1][1];j++,k++) {   
                if (dfftFrequenciesInMel[j] < dfilterCenterFrequencies[i]) {   
                    ...
                    m_dweights[i-1][k] = (float) (1.0 -( (dfftFrequenciesInMel[j]-dfilterCenterFrequencies[i]) /   
    ...
    public float[] getParameters(float[] fspeechFrame) {   
        ...
        for(int i=0; i < m_nnumberOfFilters; i++) {   
            ...
            if (m_dfilterOutput[i] > m_dminimumFilterOutput) {//floor power to avoid log(0)   
                m_dfilterOutput[i] = (float) Math.log(m_dfilterOutput[i]); //using ln   
            } else {   
